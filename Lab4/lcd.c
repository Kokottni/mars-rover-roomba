...
//Puts up an infinite loop rotating banner
void lcd_rotatingBanner(char data[]) {
    char screen[21] = "";
    uint8_t msglen = strlen(data);
    uint8_t screenlen = 20;
    uint8_t msgptr = 0;
    uint8_t i = 0;
    ...
    while (1) {
        ...
        // Add the next character from data to the end of the screen
        if (msgptr < msglen) {
            screen[screenlen - 1] = data[msgptr];
            ++msgptr;
        } else {
            screen[screenlen - 1] = ' ';
            ++msgptr;
            if (msgptr >= msglen + screenlen) {
                msgptr = 0;
            }
        }
